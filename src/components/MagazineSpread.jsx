...
const BOARD_CAPTIONS = {
  materials: (synthesis) => synthesis
    ? `כל פרויקט מתחיל בשאלה אחת: במה הוא עשוי? לוח החומרים אינו רשימת קניות — הוא הצהרת עמדה. ${synthesis.material ? synthesis.material + '.' : ''} הבחירה בחומר היא בחירה בזמן, במגע, בהגיון הפנימי של המרחב.`
    : 'כל פרויקט מתחיל בשאלה אחת: במה הוא עשוי? לוח החומרים אינו רשימת קניות — הוא הצהרת עמדה.',
  colors: (synthesis) => synthesis
    ? `הצבע הוא הטמפרטורה הרגשית של החלל. ${synthesis.tension ? `"${synthesis.tension}" —` : ''} לא מדובר בגוון בלבד, אלא בכוח שמשתנה עם שעות היום, עם האור, עם הנוכחות האנושית בתוכו.`
    : 'הצבע הוא הטמפרטורה הרגשית של החלל — לא גוון בלבד, אלא כוח שמשתנה עם שעות היום.',
  mood: (synthesis) => synthesis
    ? `לפני שהעיפרון נוגע בנייר, יש תחושה. לוח האווירה הוא ניסיון לתפוס אותה — ${synthesis.token ? synthesis.token + '.' : ''} אוסף תמונות שאינן מסבירות את הפרויקט, אלא מרגישות אותו.`
    : 'לפני שהעיפרון נוגע בנייר, יש תחושה. לוח האווירה הוא ניסיון לתפוס אותה.',
};

const ROOM_CAPTIONS = {
  living: (synthesis) => synthesis
    ? `הסלון הוא הטיעון העיצובי המרכזי של הבית. כאן הכל מתנגש — ריהוט ואדריכלות, שגרה וחגיגה, פרטיות ואירוח. ${synthesis.tension ? `"${synthesis.tension}" —` : ''} הכרעה עיצובית שמגדירה את כל היתר.`
    : 'הסלון הוא הטיעון העיצובי המרכזי של הבית — כאן הכל מתנגש ומוכרע.',
  kitchen: (synthesis) => synthesis
    ? `המטבח נבחן לא רק בעיניים אלא בידיים. ${synthesis.material ? synthesis.material + ' —' : ''} חומרים שעומדים בפני שמן, חום ושגרה. אסתטיקה שנדרשת להצדיק את עצמה מדי יום.`
    : 'המטבח נבחן לא רק בעיניים אלא בידיים — אסתטיקה שנדרשת להצדיק את עצמה מדי יום.',
  bedroom: (synthesis) => synthesis
    ? `האדריכל הטוב מבין שחדר השינה אינו רק חדר — הוא עמדה פילוסופית. ${synthesis.tension ? `"${synthesis.tension}".` : ''} כמה אור? כמה שקט? כמה ריק? כל החלטה היא הגדרה של מה שהשינה אמורה לאפשר.`
    : 'האדריכל הטוב מבין שחדר השינה אינו רק חדר — הוא עמדה פילוסופית.',
  bathroom: (synthesis) => synthesis
    ? `חדר הרחצה הוא המקום שבו העיצוב אינו יכול להתחבא מאחורי תכנים. ${synthesis.material ? synthesis.material + '.' : ''} רק חומר, אור ופרופורציה — ושלושתם נחשפים לחלוטין.`
    : 'חדר הרחצה הוא המקום שבו העיצוב אינו יכול להתחבא — רק חומר, אור ופרופורציה.',
};

const BUILDING_CAPTIONS = {
  private: (synthesis, project) => synthesis
    ? `החזית היא החוזה בין הבית לרחוב. ${project.poeticDescription ? project.poeticDescription + ' —' : ''} ${synthesis.tension ? `"${synthesis.tension}".` : ''} מה נחשף ומה מוסתר — שאלה אדריכלית שהיא גם שאלה תרבותית.`
    : 'החזית היא החוזה בין הבית לרחוב — מה נחשף ומה מוסתר, שאלה אדריכלית שהיא גם תרבותית.',
  building: (synthesis, project) => synthesis
    ? `הבניין הרב-קומותי מבקש מהאדריכל לדבר בשתי שפות בו-זמנית: שפת הדייר הבודד ושפת הרחוב כולו. ${synthesis.tension ? `"${synthesis.tension}" —` : ''} מתח שאין לו פתרון, רק עמדה.`
    : 'הבניין הרב-קומותי מבקש מהאדריכל לדבר בשתי שפות — שפת הדייר הבודד ושפת הרחוב כולו.',
};
